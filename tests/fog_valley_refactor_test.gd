...
func _initialize() -> void:
	...
	var cloudy_profile: Dictionary = main.call("_weather_profile", "cloudy")
	var mist_profile: Dictionary = main.call("_weather_profile", "mist")
	var rain_profile: Dictionary = main.call("_weather_profile", "light_rain")
	assert(rain_profile["rain"] == 1.0)
	assert(cloudy_profile["surface_wetness"] < mist_profile["surface_wetness"])
	assert(mist_profile["surface_wetness"] < rain_profile["surface_wetness"])
	assert(is_equal_approx(rain_profile["surface_wetness"], 0.78))
