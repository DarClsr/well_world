...
## 2026-09-03 Phase 82 初步画面审计

- 雾池西南 `TwistedTree_1` 是当前自然布置中唯一使用原始 `Leaves_TwistedTree_C.png` 的大树；已有 `Leaves_TwistedTree_C_Muted.png` 已在三个季节性灌木上通过色彩与 Alpha 契约。
- 规范允许环境中少量地域性色彩，但面积、明度与饱和度不能同时接近叙事焦点；大面积酒红树冠会与传送门冷青和炉火暖色争夺焦点。
- 最小方案是在现有树叶 surface 材质副本上替换已有低饱和贴图；树干、位置、缩放、风摆和雾池构图全部保持不变。
- 树冠动效通过 `_apply_tree_canopy_wind()` 将源叶片贴图传给共享 Shader；给该函数增加可选贴图参数，并仅由 `TwistedTree_1` 传入 `MutedBushLeaves`，即可保留所有风摆参数和材质边缘行为，无需复制 Shader 或增加材质管理层。
- 冒烟契约确认 16 个树冠中仅雾池 `(-16, 17)` 的树冠使用低饱和贴图，其余树冠纹理与共享 Shader 不变；树木/落叶动态回归仍为 `canopies=16 leaves=12`。
- 既有 `grass_sweep_capture.gd` 已固定 `time_hour=9.5`、天气种子和晴/小雨 override，且包含雾池岔口、雾池近景、传送门、村庄与四角共 21 个机位，可直接用于本阶段全景回归；无需新增捕获脚本。
- 隐藏 Movie Writer 固定 `5 FPS` 的晴天与小雨扫查均正常退出，并分别生成 21 张不同哈希画面；既有 GLES3 退出资源告警未影响图像或退出码。
- 人工复核 `04-pond-fork` 与 `05-pond-close`：晴天树冠已从鲜红降为灰粉酒红，雾池、角色和道路更先被读到；小雨近景仍显著偏红，需要量化并交美术专项判断，不能只凭晴天帧验收。
- FFmpeg 对 `05-pond-close` 左侧 `560x720` 同区裁切的 `SATAVG` 为晴天 `26.6813`、小雨 `37.9318`；雨天环境会显著抬高该区域色彩权重，已将晴雨双条件作为美术终审重点，而不是只验证贴图路径。
- 美术代理核对文件时间后确认上述小雨图是 Phase 81 旧证据：启动参数缺少 `--` 分隔，`--rain` 未进入 `OS.get_cmdline_user_args()`；旧图与其量化结果不能用于 Phase 82 验收，必须按正确参数重录。
- 同机位 A/B 捕获确认原始贴图在晴雨下均接近高纯度猩红，低饱和贴图在晴雨下均保持灰粉酒红；随后用 `-- --rain` 正确重录的小雨 21 张文件时间全部更新到 `16:17:47-16:18:04`，且保持 `21/21` 唯一哈希。
- 当前 `04-pond-fork` 晴雨图中树冠只作为左缘极小点缀；`05-pond-close` 晴雨图中树冠均为低饱和灰粉酒红，雾池水纹、冷白灯菇和玩家轮廓仍先于树冠可读，旧雨天高纯度红色问题不再出现。
- 三类最终终审均为 `PASS 0/0/0`：树冠保留扭曲轮廓与酒红地域色，晴雨下均退为次级框景；连续帧无白边、跳闪或树干漂移，住宅、道路、NPC 与生活职责空间不受影响。

## 2026-09-03 Phase 83 初步动效审计

- `_animate_ecosystem()` 当前将蝴蝶 Alpha 完全取自 7:00-19:00 时段曲线，小雨时仍保持 7 只全部可见，缺少基本天气响应。
- `weather_rain_amount` 已由天气 profile 连续插值并用于炉火、烟雾、NPC 避雨和地表反馈；蝴蝶只需在最终 Alpha 乘以 `1.0 - weather_rain_amount`，不需要增加状态或控制器。
- 数量、固定种子、路径参数和扑翼动画无需改变；萤火虫为夜间冷绿生态，与本阶段白昼小雨响应无关。
- `light_rain` profile 的 `rain=1.0`，晴天为 `0.0`，`blend_weather_profiles()` 已对雨量做线性连续插值；因此乘法会在完整小雨时隐藏蝴蝶，并在过渡中自然淡入淡出，不需要阈值或额外 Tween。
- 冒烟契约已实测中午晴天 Alpha `1.0`、半雨量 `0.5`、完整小雨 `0.0`，且萤火虫保持隐藏；雾谷重构基线通过，说明改动没有越过生态显示边界。
- 固定中午生态 A/B 中，晴天两帧可见少量浅米色蝴蝶并保持游走，完整小雨两帧无蝴蝶但雨线、角色、道路和草甸正常；没有通过删减数量或关闭整个生态节点实现。
- 改后晴天与正确传入 `-- --rain` 的小雨全景扫查均为 21 张本轮时间戳、21 个唯一哈希；蝴蝶天气响应未冻结场景或影响捕获稳定性。
- 三类终审均为 `PASS 0/0/0`：晴天蝴蝶数量与画面面积克制，小雨淡出后场景仍由雨线、风摆、NPC 和职责节点保持活力；隐藏期间路径相位继续更新，雨停不会出现位置弹跳。

## 2026-09-03 Phase 84 全景基线审计

- 将最新晴天与小雨各 21 机位拼成统一联系表后，出生石环、道路、雾池、三栋住宅、炉火、药圃、板车和边界树群的主次关系整体稳定；当前没有一眼可确认的 P0/P1，不应凭空增加道具。
- 下一步应等待美术、生活气息和动效三类独立提名，优先选择有具体机位与可复现行为证据的单一缺口；山口和北侧雾幕继续排除。
- 生活气息提名：Nia 的 `rain_shelter` 目前仅在 18:30-22:30 生效，白天小雨仍在露天布架作业；若处理，必须增加连续离岗/返回状态，不能瞬移或穿屋。
- 动效提名：三块挂布与四束晾药都在既有 `wind_nodes` 中，但 CPU 风摆固定使用 `0.027/0.013` 振幅，没有读取晴天 `0.62` 与小雨 `1.12` 风强；只需乘受限相对风强即可保持现有节点和动画结构。
- 美术审核未提名新的视觉缺口，并明确不应增加静态道具、植被、魔法特效或全局调色；Phase 84 因此选择已有物件天气风摆 P2，不新增可见内容。
- 风摆位于 `_runtime_tick()` 且发生在 `_apply_time_of_day()` 更新天气参数之后；用 `clampf(weather_wind_amount / 0.62, 0.75, 1.45)` 可让晴天保持原幅度、薄雾略弱、小雨克制增强，同时避免按完整 `1.12/0.62` 放大到约 1.8 倍。
- 冒烟在同一 `portal_time=3.25` 下分别采样挂布与药束，确认完整小雨摆幅均严格为晴天的 `1.45` 倍；两组立柱、横杆及药架雨棚的 Transform 全部保持不变。
- 专项捕获固定 `time_hour=9.5`、天气种子及 `portal_time=2.0/3.25`；第二版外侧固定相机让挂布与晾药束完整可见，A/B 能读出摆动但没有越出横杆、打入房墙或穿过药架雨棚。
- 晴天/小雨专项共 8 张且 `8/8` 唯一哈希；全景扫查分别为 `21/21` 唯一哈希，生成时间均为本轮，排除隐藏窗口冻结和旧雨图复用。
- 人工复核中雨天风摆差异保持次级：炉火、道路、角色和房屋仍是更强的构图锚点；天气冷灰层级没有因物件动效新增高饱和或高亮焦点。
- 美术终审 `PASS 0/0/0`：挂布与药束仍为低对比背景细节，灰绿、木棕与低饱和布色符合异世界乡野层级，未见穿模或整架摇动。
- 生活气息终审 `PASS 0/0/0`：挂布配合织工、药束与雨棚配合药圃，职责空间更像持续使用中的生活区域；主路、入口、板车和 NPC 活动空间不受影响。
- 动效终审 `PASS 0/0/0`：A/B 错相清楚且克制，布片中心枢轴在当前约 4 度以内峰值不产生可见脱钩，药束外层锚点自然；`1.45` 上限无需降低。

## 2026-09-03 Phase 85 状态审计

- 当前 `_update_nia_routine()` 只在 `18:30-22:30` 将小雨切换为 `rain_shelter`，因此 06:30-18:30 的常态 `work` 仍会在晾织架露天播放 `Interact`。
- 已有 `NIA_RAIN_ROUTE` 从织工院附近沿村庄公共空间通往东屋门前，终点 `(8.6, -4.2)` 已通过夜间小雨捕获；无需新增遮棚、路径系统或视觉内容。
- 直接从东屋避雨点切回 `work` 会让工作路线从远端斜穿村庄，存在穿屋/卡碰撞风险；需要显式反向返回状态，抵达织工院后再恢复 `work`。
- 已经进入 `errand/returning` 的短时板车事务应先沿原路线完成，再响应避雨；否则从公共路线任意点切入另一条路线可能斜穿职责区，也会破坏已验证的卸货叙事。
- 交谈冻结位于 `_physics_process()` 的 Nia 状态更新之前，新增状态可以自然继承交谈优先级，不需要额外对话分支。
- 实现前生活审核指出东屋避雨虽已验证，但离织工院约 17 米且横跨主路；白天更自然的目标是复用自己住宅外侧 `NIA_HOME_ROUTE[-1]` 代理点，不新增雨棚。
- 动效审核指出反转旧 `NIA_RAIN_ROUTE` 不会回到工作锚点，且新增返回态必须优先于新事务窗口、在终点完整重置工作索引；据此改用闭合的两点白天短路线并锁定状态优先级。
- 状态顺序以 22:30 回屋为硬上限；已经开始且未完成的 `errand` 及其 `returning` 在此前优先完成，晚间逻辑其次，白天晴雨离岗/返回再次，新事务最后，避免每帧被时段分支打断。
- 两点路线若沿用“选择最近端点”，天气在半途变化时可能先走向较近旧端点再折返；白天避雨与返回状态切换应直接以各自最终端点为目标，才能保证停雨/复雨立即沿原线反向。
- 首轮复雨契约只推进一帧，角色距避雨点约 `0.012m`，会被现有 `0.12m` 到达阈值正常吸附并切 `Idle`；有效的半途复雨验证必须先让角色离开该阈值，而不是放宽动画断言。
- 冒烟环境中手工调用 `_physics_process()` 时，`move_and_slide()` 的实际单步约 `0.108m`；半途测试不能用固定多帧数猜测位置，应以角色同时离开路线两端到达阈值作为采样条件。
- Movie Writer 的 `_capture()` 还会等待 `0.8s` 稳定画面；对白天约 1 米返岗路线而言，这足以让 Nia 接近终点并切回 `work`。专项动作证据必须在预定采样点暂停主场景物理，再等待渲染帧，不能用最终日志反推较早状态。
- Nia 从晾织架实际操作点 `(-4.7, 6.35)` 到住宅代理点 `(-6.5, 6.4)` 约 `1.8m`；两点状态路线的工作锚点 `(-5.5, 6.5)` 用于状态闭合，不代表所有工作姿态都从该点起步。
- 第三轮专项图中离岗/返岗均位于织工院内且未侵占主路，但默认东南相机让住宅遮挡了屋侧避雨终点；终审证据必须换侧向机位让角色、屋檐和晾织架同时可读，不能仅凭状态日志通过视觉门禁。
- 东侧 `yaw=90°` 机位能把 Nia 放在住宅墙面前景，屋檐关系明确，角色与名字完整；玩家距 Nia 约 `3.8m`，没有触发交谈，池边与板车支路仍提供生活环境上下文。
- Phase 85 晴天/小雨 21 机位联系表保持既有低饱和乡野环境、土路、草甸、住宅和职责物件层级；小雨没有新增透明排序、屋顶异常或道路占用，传送门与炉火仍是受控冷暖焦点。
- Phase 85 美术终审 `PASS 0/0/0`：鼠尾草绿/赭石色板、细雨、角色比例、屋檐关系和冷暖叙事焦点符合规范；联系表底部黑区只是 21 图按 4x6 拼版的空槽。
- 生活气息终审 `PASS 0/0/0`：Nia 就近退到自宅屋侧，路线留在织工院内；晾织架、入口、支路和主路均保持净空，事务/晚间/回屋优先级合理。
- 场景动效终审 `PASS 0/0/0`：最终日志与三张证据一致，离岗 `Walk`、檐下 `Idle`、返岗 `Walk`；停雨/复雨立即反向，抵达工作锚点后才恢复 `work`。

## 2026-09-03 Phase 86 候选审计

- `_physics_process()` 每帧无条件调用 `_update_mira_herb_route()`，而旧实现只判断雨量、不读取 `time_hour`，因此米拉在 22:00 与午夜仍会巡走并播放 `PickUp`。
- 生活气息与场景动效审核均将此列为下一处 P1；美术审核提名的草甸三档轮廓为 P2，且要求不减少实例、不改位置/颜色/排除带，后续独立处理。
- 米拉可复用 `MIRA_HERB_ROUTE`、`MIRA_RAIN_SHELTER_APPROACH` 与 `MIRA_RAIN_SHELTER`：从当前药圃路段先反向回到 route[0]，再到屋檐；返工按完全相反顺序，能避免跨药圃或斜穿住宅。
- 交谈冻结已经位于米拉状态更新之前，无需新增对话分支；隐藏 NPC 必须在外层物理循环跳过 `move_and_slide()`，否则碰撞 mask 归零后会在屋内持续向下坠落。
- WIP 的首个冒烟失败并非旧断言过时：晴天 `else` 分支把 `villager_patrol_pauses[0]` 在稳态 `work` 中也清零，开场 1.8 秒停留无法成立，米拉随即播放 `Walk`。计时清零应只发生在真正取消 `homeward` 的转换点。
- Phase 86 首轮四帧状态日志均正确，但沿用 Phase 73 相机中心时，黄昏返家与上午采药的米拉被左边界裁切；这两帧不能作为动作证据。相机高度和朝向保持不变，仅将中心从 `x=-3.7` 西移到 `x=-7.0`，同时容纳药圃、住宅屋檐和往返路线。
- 西移后的四帧同时容纳药圃、米拉住宅、屋檐、板车支路和主路：黄昏/清晨的 `Walk` 与上午 `PickUp` 角色完整，23:00 住宅外和药圃均无米拉；夜景中窗光与炉火先于环境被读到，未引入额外高饱和焦点。
- Phase 86 晴天与小雨联系表未见道路、屋顶、住宅或药圃回归；屋顶保持不透明，无半透明叠层，昼夜与晴雨焦点层级符合 `docs/art-direction.md`。
- 美术、生活气息与场景动效终审均为 `PASS 0/0/0`。米拉路线保持在药师职责区，状态与 `Walk/Idle/PickUp` 连续，隐藏后不参与物理移动，既有树冠、落叶和天气动效未回归。

## 2026-09-04 Phase 87 实施边界

- 美术审核定位的剩余 P2 是远景单一草模型形成同高、同字形排线；规范明确草甸体量应优先用高度或形态变化维持节奏，不应据此减少总量。
- 当前落点、数量、旋转和明度由固定种子 RNG 顺序决定；轮廓档位可使用已接受实例序号取模，避免增加随机调用而改变后续位置或颜色。
- 最小实现是在现有非均匀 Basis 缩放中加入三档宽高倍率，不增加模型、材质、Shader、节点或配置层；现有最大基础缩放 `0.34` 乘 `1.12` 后仍低于既有包围盒和角色遮挡上限。
- 改动前后均为 `1849` 株，位置数组哈希 `180955129`、明度数组哈希 `3492265916` 完全一致；三档数量为 `617/616/616`，不存在通过删草或重排落点制造差异。
- Dummy/Headless 渲染器下 `MultiMesh.get_instance_transform()` 返回单位变换，不能验证真实 Basis；轮廓档位改由运行时元数据锁定，最终视觉必须以 Compatibility 截图判断。
- 晴天三机位左旧右新 A/B 初审显示大簇平顶线被轻微打散，变化保持克制；道路边缘、玩家、守卫、炉火与住宅入口未被更高草叶遮挡。
- 晴天与小雨完整联系表均未出现均匀草毯、活动区侵入或高窄草深色噪声；药圃、炉火、住宅门前和道路排除带继续保持清楚留白，传送门与炉火仍是冷暖焦点。
- 三类专项终审均为 `PASS 0/0/0`：美术确认远景同高排线已被克制打散，生活气息确认职责区与道路留白未受侵入，动效确认静态 Basis 档位不影响世界错相风摆、天气倍率或镜头距离衰减。
- 本阶段没有引入可进一步删除的辅助层、资产或依赖；直接复用现有 MultiMesh 实例构建与节点元数据，是保持固定随机序列和自动契约的最小实现。

## 2026-09-04 Phase 88 当前画面初审

- Phase 87 晴雨联系表中，地面、道路、草甸、房屋和冷暖焦点的整体层级稳定，没有足够证据支持全局调色、减少植被或增加新资产。
- 草甸在村庄北侧、远景和主路三类尺度已打散同高轮廓，活动区留白仍清楚；不应继续围绕草量或草形反复微调。
- 当前仅把屋顶避让后的室内舞台空度、以及 NPC 昼夜/天气职责合理性列为待独立复核方向；在专项审核给出具体机位或状态证据前不做实现判断。
- 下一阶段继续排除山口与北侧雾幕，避免把序章出生区的局部完善误扩张为新地图开发。
- 代码复核确认三栋住宅已分别绑定 `herbalist/hearth/weaver` 室内身份，并包含暗色地板、差异化地毯、长凳、木箱与职业陈设；`tests/smoke_test.gd` 已锁定其尺寸、位置、颜色和无碰撞属性。没有具体镜头证据时，不把“室内可能偏空”当作成立缺口。
- 屋顶目前只对瓦面与烟囱做带双重滞回的平滑避让，墙壳保持不透明；相关材质 Alpha、三栋独立状态和退出滞回已有冒烟覆盖，不应重新采用整栋透明方案。
- 动效审核未找到当前场景的新缺口，并认为守门人夜巡符合职责；生活气息审核则将托伦全天三点巡守列为 P1。由于两者对同一事实结论相反，且缺少夜间专项成片证明其破坏场景叙事，本阶段不修改托伦。
- 美术审核在固定晴雨 `03-portal-west` 发现一致 P1：玩家位于合法 4.5 米调查站位、镜头旋转 90 度时，九块全部处于 `z=0` 的拱石侧向重叠为实心柱，玩家只剩兜帽，冷青石环也失去环形地标辨识度。
- 该问题同时违反“角色轮廓不能被地标吞没”和“镜头旋转后仍有可识别地标”；默认 `01-portal-near` 正面构图正常，因此应只打散拱石侧向投影，不引入透明避让或重做传送门。
- 第一轮采用 `-0.30..0.34m` 交替错位后，晴雨西侧与晴天东侧实机画面仍只露出玩家兜帽；虽然实心柱边缘已被打散，但岩石自身横向体积仍覆盖中心投影，因此该方案未达验收，不能保留为完成状态。
- 第二轮改为左右拱臂分别位于约 `z=-0.50..-0.68m` 与 `z=0.48..0.66m` 两层，让侧视中心形成连续角色缝隙；前后变化仍只作用于原九块拱石，且中心、碰撞与魔法节点不变。
- 第二轮正面近远景仍清楚，但西/东侧画面中的玩家依旧仅露兜帽，说明岩石横向体积继续覆盖 `z=0` 中央投影；不能把“不规则柱形”误判为角色可读性通过。
- 第三轮保持两块落地拱脚约 `z=-0.54/0.58m`，只让不可达的中上段沿高度逐步展开至 `z=±1.15m`，尝试形成真实三维扭转而非九块共面石墙；若同机位仍失败，将放弃继续放大静态偏移。
- 第三轮侧视仍只露兜帽，且正面开口出现不必要的青色斜缝，证明静态深度偏移不是正确根因修复；九块拱石应恢复共面，改为仅在石环确实位于相机到玩家线段内时隐藏上部遮挡视觉。
- 新遮挡契约首次失败发生在后续既有门面初值断言：专项测试曾让玩家靠近石环并运行 `_process()`，`reaction_strength` 按设计变为非零。修正仅清理测试产生的接近反馈，不删除或放宽原断言。
- 首版视线遮挡在晴雨东西两侧均完整显露玩家，且正面近远景保持原构图，证明窄迟滞判定覆盖了真实失败条件；但同时隐藏环体与门面会让侧向地标只剩地面符文，异界识别不足。
- 最终候选进一步收窄为只隐藏九块石质拱架，冷青环体、门面、地面符文、微光和灯光继续可见；侧视应保留一条克制魔法轮廓，同时避免重新遮住角色主体。
- 只隐藏拱石的实机结果中玩家完整、正面不变，但边缘朝向镜头的实心门面显示为过长、过实的青色竖条，读感接近激光而非古老遗迹；最终候选将门面同拱石一起隐藏，只保留青色环体、环形投影、地面符文、微光和灯光。
- 进一步只保留环体后，侧视仍形成同样的长青色竖条，确认环体边缘投影也不能承担地标识别；最终遮挡组恢复为九块拱石、环体和门面共 11 个视觉节点。窄遮挡状态仅保留圆形地面符文、漂浮微光、灯光与调查提示，离开迟滞带立即恢复完整石环。
- 最终专项捕获固定 9:30 和天气种子：东西侧遮挡机位中玩家的兜帽、斗篷、躯干与双腿连续可辨，地面符文、微光及提示仍定位异界交互；正面近景/远景未触发遮挡，完整破损石环与冷青门面保持原构图。
- 最终晴天、小雨全景均为本轮 `21/21` 唯一哈希，且 21 对同名晴雨画面全部不同；录制日志均到达 `SAVED 21-corner-sw`，没有脚本或解析错误。
- 最终并行录制清单漏掉了专项小雨进程，导致专项雨目录仍是“仅保留环体”的上一候选；两套 21 机位小雨不受影响，但专项东侧雨图与前后对照雨天右图必须作废并重录，不能用文件存在代替构建版本核对。
- 恢复后使用显式 `-- --rain` 静默重录专项小雨，进程退出码为 `0`，日志依次到达 `SAVED west/east/near/far`；四张文件时间均为本轮且哈希互异。
- 新小雨西/东侧画面中玩家从兜帽到双腿连续可辨，石环上部 11 个遮挡视觉节点全部让开；正面近/远景中完整石拱与门面仍存在，证明最终规则没有退化为按距离永久隐藏。
- Compatibility 捕获退出时仍报告既有 GLES 资源释放告警，但四张 PNG 均为正常 1280×720 非空画面且进程退出码为 `0`；该告警不作为视觉失败。

## 2026-09-04 Phase 88 完成

- 旋转迟滞专项以合法西侧调查站位单向扫描 `0..180°`，结果为 `transitions=2`、`hidden_samples=29`：只发生一次进入和一次退出，没有边界 chatter，也没有全程常隐。
- 最终归档已覆盖正确构建：晴雨侧向前后对照、晴雨东侧、正面近景/远景及两套 21 机位总览；原侧视石柱吞没玩家 P1 关闭，正面破损石环与冷青门面无回归。
- 美术、生活气息与场景动效三类终审均 `PASS 0/0/0`。托伦作息仍因先前审核结论冲突而不在本阶段处理，没有借机扩张范围。
- 最终门禁为 `SMOKE TEST PASSED`、`TREE MOTION TEST PASSED canopies=16 leaves=12`、`PORTAL HYSTERESIS TEST PASSED transitions=2 hidden_samples=29` 和 `git diff --check`；仅保留既有 glTF UID fallback 与退出资源告警。

## 2026-09-04 Phase 89 基线

- 当前权威提交为 `17166a0`，工作树起点干净，后台 Godot PID `47888` 正常响应。
- Phase 88 后晴天与小雨扫查目录各有 21 张图，时间范围均属于最终录制，两套分别为 `21/21` 唯一哈希。
- 美术规范仍要求用少量有叙事作用的焦点维持俯视舞台构图、角色轮廓和村庄生活归属；不得把持续迭代误解为继续堆资产。
- Phase 89 先收集三类独立证据，只实施一项最高优先级缺口；此前结论冲突的托伦作息不能在没有新证据的情况下直接沿用为结论。
- 主代理逐张复核村庄与生活区机位后发现待验证候选：晴雨 `14-herb-yard` 中屋顶已隐藏，但玩家位于住宅外侧时仍被保留的侧墙遮住大半，只剩兜帽和局部身体；同批 `15-west-house` 角色完整，说明问题与特定相机到玩家视线关系有关，而不是角色比例或全局镜头错误。
- 该候选若成立，应优先解决角色可读性，不能简单恢复整栋透明，也不能违反规范中“墙壳保持不透明”的既有方向；可能需要只给被建筑遮挡的玩家提供克制轮廓，而不是继续扩大房屋隐藏范围。
- Phase 89 三方审计收敛：美术与生活气息均以晴雨 `14-herb-yard` 提名同一 P1，动效审核为 `none` 并明确不应再改草木、天气、炉火或 NPC 动效；因此本阶段只处理玩家被住宅侧墙遮挡。
- 美术审核曾先提名主路规则性，随后在核对 `14-herb-yard` 后将最终最高优先级更新为角色遮挡；采用其最新结论，主路本阶段不再打开。
- 第一轮原生 `StandardMaterial3D` 遮挡轮廓探针成功在 Compatibility 下透过墙壳显示完整玩家，但浅灰绿色 `RGB≈0.50/0.58/0.55`、Alpha `0.24/0.36` 呈现明显幽灵感；技术路径成立，色值需改为更暗的低饱和绿灰后再选择强度。
- 第二轮探针使用暗绿灰 `RGB≈0.22/0.32/0.29`；Alpha `0.30` 已能在浅色墙壳上读出完整角色且不压过炉火与环境，`0.42` 视觉偏重，因此生产上限选 `0.30`。
- 生产实现不新增 Shader 或射线框架：给玩家全部现有网格共享一个 `StandardMaterial3D` 覆盖层，使用 `no_depth_test + cull_front + grow_amount=0.035`，并只在任一既有 `house_roofs_faded` 状态成立时将 Alpha 平滑推向 `0.30`。
- Phase 89 晴天/小雨 21 机位均完成重录，分别为 `21/21` 唯一哈希；联系表人工复核未见石环、雾池、道路、炉火、NPC、树木或其他住宅机位误触发轮廓。
- `14-herb-yard` 晴雨画面均显示暗绿灰完整玩家轮廓，墙壳保持不透明；无遮挡控制帧 `15-west-house` 保持原角色材质，没有常驻描边。
- 专项补录覆盖 9:30 晴天、9:30 小雨、23:00 晴天和无遮挡控制帧；夜间轮廓仍低于炉火的视觉权重，未变成高亮魔法效果。
- 前后对照从 Phase 88 晴雨联系表的同一 `14-herb-yard` 机位裁取旧帧，并与 Phase 89 原尺寸新帧缩至相同尺度组合；上排晴天、下排小雨，左右分别为旧/新。
- 新轮廓行为已写入 `docs/art-direction.md` 建筑规范：墙壳保持不透明，只有真实住宅遮挡时允许暗绿灰玩家轮廓平滑出现，无遮挡必须完全隐藏。
- 主代理复核最终证据：晴雨前后对照中修复后的兜帽、斗篷、背包和四肢形成连续暗绿灰轮廓；23:00 轮廓仍弱于炉火，无遮挡控制帧无残留描边；晴天 21 机位总览未见效果扩散到其他角色、建筑或环境物件。
- 美术、生活气息、场景动效三类终审一致为 `PASS 0/0/0`，住宅墙壳、室内陈设、药圃、道路和夜间安全岛叙事层级均未回归。
- `Parameter "material" is null` 不是提交 `17166a0` 的既有日志：独立 worktree 基线只有既有 Dummy 资源释放告警。本轮根因是无头测试直接销毁仍绑定共享 `material_overlay` 的导入网格；冒烟和树冠测试在退出前先解绑覆盖材质并释放主场景后，该告警消失。
- 树冠回归的实际入口是 `tests/tree_motion_capture.gd`；`tests/tree_motion_test.gd` 不存在，后续不得继续沿用该旧文件名。

## 2026-09-04 Phase 90 当前画面初审

- Phase 89 最新晴天 `06-road-mid-north`、`08-village-north`、`10-village-south` 与 `16-north-road` 实图中，雾池分叉后的主路在住宅核心段持续保持近似相同宽度与方向，形成贯穿画面的浅色直带，三栋住宅被读成道路两侧的规则排列。
- 该问题直接对应 `docs/art-direction.md` 的道路规范：住宅核心段应适度收窄并轻微摆动，支路应从路缘自然散开，不能以连续等宽浅色带切开建筑；美术专项将其评为 Phase 90 当前最高优先级 P1。
- 最小候选只调整 `VillagePath` 在雾池分叉至村庄北端的横向轮廓与半宽，保持中心通行、碰撞安全宽度、房门支路、板车磨损、炉火、建筑和 NPC 位置不变；道路路肩与草甸排除边界必须随最终网格同步。
- 生活气息审查与美术审查独立复现同一 P1；场景动效审查为 `none`，并明确草木、天气、生态、炉火与 NPC 动效当前稳定，因此 Phase 90 不触碰动态参数。
- 美术代理复核候选时指出 `z=-5` 若同时左移到 `x=-1.05` 并缩至 `1.08m` 半宽，会削弱炉火支路接缝；最终保留接近原中心的 `x=-0.78` 与 `1.20m` 半宽，只靠相邻控制点形成轻摆。
- 最小实现继续复用 `_add_road()` 和 `meadow_roads`：核心半宽调整为 `1.25/1.20/1.12/1.28/1.70m`，控制点只作 `0.10-0.23m` 级别横移；入口最大完整宽度仍为 `4.2m`，草甸排除边界自动跟随采样后的道路轮廓。
- 最终晴雨 A/B 与总览确认道路形成克制、连续的不对称轻摆，没有蛇形折点；炉火、房门、药圃和板车支路保持视觉连接，道路重新退回环境引导层。
- 道路收窄释放的空间令草甸由 `1849` 株增至 `1902` 株，未改变固定种子、密度、颜色或风摆逻辑；树冠/落叶测试与动效终审确认没有形成路缘草墙或新增动态噪声。
- 美术、生活气息和场景动效三类最终审查均为 `PASS 0/0/0`；Phase 90 的原始主路 P1 已关闭，没有新增 P2。

## 2026-09-04 Phase 91 基线

- 权威提交为 `2074639`，工作区干净；后台试玩 PID `26364` 正常响应。
- Phase 90 晴雨 21 机位、道路 A/B、树冠与落叶证据均已归档且通过三类终审；住宅核心主路不再作为开放问题。
- 当前美术规范强调少量高质量焦点、完整互动事件与连续路线，不允许为了持续迭代而无证据增加装饰；本阶段先独立审图再确定唯一实施项。
- 美术与动效审查均未发现新的 P0/P1/P2；生活气息唯一提名为 `16-north-road` 的托伦职业空间 P2：去掉姓名标签后，他只是站在住宅北侧草肩上的普通 NPC。
- 最小实现是 `VillageProps/TorenWatchPost`：一段横向短栏、一根错位木桩和一块低矮石基，位于北路东侧草肩；不增加岗亭、门楼、旗帜、灯光、碰撞或散乱道具。
- 首轮位置 `z=-19.15` 被镜头上沿截断，第二轮纵向栏杆只呈细线，第三轮虽改横向但与姓名标签重叠；最终位置 `(5.6, 0, -18.3)` 和横向短栏在 `16-north-road` 中完整可读，位于标签上方且不侵占主路。
- 最终晴天与小雨 `16-north-road` 均确认木栏、错位桩和石基保持低饱和、无明显草叶或雨线穿插；炉火、角色和道路仍先于值守点被读到。
- 晴雨扫查均为 `21/21` 唯一哈希，21 对同机位无相同帧；完整冒烟与树冠/落叶回归通过，既有 UID 和退出资源告警未影响退出码或证据。
- 美术、生活气息、场景动效三类最终审查均为 `PASS 0/0/0`：值守点关闭了托伦职业空间 P2，未扩张为门楼或新增动态系统。

## 2026-09-04 Phase 92 基线

- 当前权威提交为 `1fb3dc5`，本地 `main` 与 `origin/main` 一致，后台试玩 PID `55772` 正常响应。
- Phase 91 晴雨总览、托伦前后对照和树木动效证据均已归档；三类终审为 `PASS 0/0/0`，托伦值守点不再作为开放问题。
- `docs/art-direction.md` 仍称“现有 Quaternius Ranger”可作为验证资产，但生产主角已经是 KayKit Hooded Rogue 改造的漂泊者；该条属于待核对的权威规范漂移，不能继续当成当前角色事实。
- 当前 `prologue_arrival.tres` 只包含调查石环、遇见托伦、抵达炉火三步；`main.gd` 也只在石环对话、托伦对话和炉火范围推进这三步。规范要求的“认识村民并完成一次活动”尚无玩家可完成的职责活动，NPC 自己采药或织作不能替代玩家互动。
- 因此若最新成片没有新的 P0/P1，下一步应优先比较“补一次小型玩家活动”与纯视觉 P2，而不是继续增加静态装饰；具体活动仍需先核对现有交互、任务和世界状态边界。
- 三类审查最终只有生活气息提出 P1：玩家没有亲手完成任何村民活动；美术与动效均为 `none`，继续堆场景装饰没有证据基础。
- 最小闭环是“见过米拉后采集药圃现有药草”：`DialogueRunner` 已能设置 `WorldState.flags`，`QuestRuntime` 严格校验步骤顺序，`collected_ids` 已有保存/恢复能力，现有玩家模型也具备非循环 `PickUp`。
- 药草目标应比同处药圃的米拉对话具有更高交互优先级，但只在 `mira_met` 且未采集时启用；采集完成后隐藏该株、写入固定收集 ID 并发出 `herb_gathered`。
- `player.gd` 当前每个物理帧都会切回 `Idle` 或 `Run`，必须增加最小的交互动画剩余时间；锁定期间仍可保留物理移动，但不得覆盖 `PickUp` 动画。
- 第二轮专项三帧确认交互在俯视尺度可读：采前右垄上方绿叶植株完整可见，动作中漂泊者侧身弯腰且目标株消失，动作后恢复直立；其余九株、晾药架、住宅和米拉活动范围不变。
- 米拉台词现以“雾叶草”提供异世界语义并自然提出一次采集请求；提示继续复用既有底部 `F` 交互样式，没有新增任务面板或高饱和标记。
- 完整游戏由 `DialogueRunner.set_flags` 写入 `mira_met`；独立 `main.tscn` 没有对话运行器，因此硬编码米拉兜底也必须写入同一标记，否则直接运行主场景时永远无法采药。
- 动效终审指出首版生产采集只播放 `PickUp` 而不改变玩家朝向，证据脚本又手工预转向，导致背对药草交互时会朝空地采集；最小根因修复是在 `_gather_herb()` 隐藏目标前按目标 XZ 方向设置 `Player/Visual.rotation.y`。
- 动效复审确认生产逻辑会自动朝向药草，`PickUp` 中段与采后恢复均清楚，证据脚本不再预设朝向；最终 `PASS 0/0/0`。
- 美术和生活气息终审同样为 `PASS 0/0/0`：任务植株保持低饱和且可辨，采后空缺自然，玩家参与米拉职责活动的闭环成立，没有扩张为背包或物品系统。
- 简化审查确认新增逻辑直接复用既有 `InteractionTarget`、`QuestRuntime`、`WorldState.collected_ids` 与玩家动画，不需要新增管理器、服务、资产或通用层。

## 2026-09-04 Phase 93 基线

- 当前权威提交为 `ae4abff`，`main` 与 `origin/main` 一致，工作树干净；后台试玩 PID `63896` 正常响应。
- `prologue_arrival.tres` 已为四个步骤提供 `description_key`，`QuestRuntime` 也发出 `quest_started`、`quest_advanced` 与 `quest_completed`，但当前场景没有消费这些数据来显示玩家的下一目标。
- 美术规范第 8 节允许轻薄的旅行手记式 UI，要求避免大面积遮挡；因此若引导成立，应使用单行短文本和低对比底板，而不是任务面板、世界箭头或高饱和标记。
- 美术规范第 7 节仍把 Quaternius Ranger 描述为现有验证资产，与当前生产主角 KayKit Hooded Rogue 改造的“漂泊者”不一致，应作为本阶段的权威规范校准，不再保留错误现状描述。
- 最新总览与采药三联图显示左上角地点标题约 4 秒后消失、底部交互反馈约 3 秒后消失；跨越四个空间节点时，当前没有持久线索，单靠一次对白不足以支持玩家绕路后恢复方向。
- 生活气息审查确认 P1：托伦未提米拉或西侧药圃，采药反馈也未接到炉火，内部步骤门控正确但世界内叙事接力断裂。
- 动效审查确认 P1：炉火只在后台完成任务，没有文字、动作或声音反馈，也没有把序章线索交给北方山口。
- 美术审查建议复用 `PersistentUI` 放置左上角单行目标，使用约 `15px` 暖灰文字、暗绿灰小菱形和 `0.78` Alpha；状态变化仅做约 `0.2s` 淡入，不使用卡片、箭头、光柱或闪烁。
- 三方意见合并为一项“序章叙事引导”：轻量目标负责离开现场后的长期线索，托伦、采药和炉火三处短文本负责世界内接力；炉火已有动态，不再叠加新特效。
- 首轮 1280×720 六状态实图中，后五帧的目标与底部对白层级成立，完成帧也正确隐藏目标；但首帧目标位于 `y=100` 时与仍在淡出的地点副标题过近，需下移后重录，不能把拥挤首帧作为最终证据。

## 2026-09-04 Phase 93 完成

- 目标行下移到 `y=124` 后重新录制，初始地点副标题与目标之间留出清晰间距；石环、米拉、炉火和完成状态均未遮挡角色、交互提示或场景焦点。
- 动效终审发现 `ObjectiveText` 默认鼠标过滤可能截断玩家滚轮缩放；文字节点现显式使用 `MOUSE_FILTER_IGNORE`，冒烟与序章集成均锁定该真实属性，复判为 `PASS 0/0/0`。
- 美术终审指出规范误称当前模型已有旧背包；现状已校准为帽兜、短斗篷和青色异界标记，旧背包保留为后续原创升级项，复判后不再把未实现特征当作基线。
- 简化审查确认实现直接复用 `PersistentUI`、`QuestRuntime` 信号与现有任务资源；没有新增框架、依赖、世界标记或特效系统。

## 2026-09-04 Phase 94 基线

- 最新晴雨 `21-corner-sw` 中，玩家位于 `(-17, 1, 9)`、相机朝向 `-135°` 时，`SeasonalBush1` 位于玩家到相机的近似视线上，躯干、斗篷、手臂和双腿均被冠幅遮住，只剩兜帽。
- 同一遮挡在晴天和小雨下位置及比例一致，属于静态构图 P1；生活气息与场景动效没有提名新的缺口。
- 西侧不可达碰撞岩壁覆盖 `x <= -24.5`。将灌木中心从 `(-18, 8)` 移到 `(-24.7, 10.3)`，可让它加入边界树岩簇，同时离开西南相机到玩家的视线，不需要全局植被透明。

## 2026-09-04 Phase 94 完成

- 晴天和小雨 `21-corner-sw` 中主角从兜帽到双腿均连续可辨；旧位置留下的空间仍由草痕、灯菇、常绿树、红叶树与岩壁形成完整疏密层次，没有边界空洞。
- 目标灌木仍保持 `0.9` 缩放、原旋转、灰粉酒红材质及天气风摆，只改变一处位置；其余 20 个机位未见新遮挡或焦点回归。
- 左旧右新的对照及晴雨最终帧归档为 `captures/phase94-southwest-*`；三类专项终审全部为 `PASS 0/0/0`。
- 简化审查确认单点数据调整和三条位置契约已覆盖问题，不需要新捕获框架、植被透明 Shader 或运行时遮挡系统。

## 2026-09-04 Phase 95 基线

- 当前权威提交为 `6efe314`，`main` 与 `origin/main` 一致且工作树起点干净；后台试玩 PID `63896` 正常响应。
- 最新晴雨扫查时间为固定 9:30，Phase 94 后西南灌木遮挡已关闭，主角、传送门、雾池、村庄主路和生活节点均可辨。
- 主代理初看尚无足够证据直接修改；待三类专项审查比较村庄建筑与道路占比、屋瓦视觉权重、生活区层次和动态叙事后，只选择一个可复现问题。
- 三方审查结果为：动效 `none`；美术认为住宅核心主路仍偏宽直；生活气息发现更确定的 P1，采药及炉火完成后米拉仍重复请求一株已经永久隐藏的雾叶草。
- 代码核对确认 `_show_villager_dialogue()` 无条件启动 `mira`，该对话还会重复写入 `mira_met` 并发出同名事件。修复应区分采集前、采集后待赴炉火、序章完成后三个状态，复用既有 `collected_ids` 和 `arrival` 任务状态。
- 道路问题与 Phase 90 同源且需要独立实图调形，本轮不与对话状态修复混合；保留为下一轮复审候选。

## 2026-09-04 Phase 95 完成

- 米拉现在按现有世界状态选择三条对白：采集前发布请求；采集后感谢并引向炉火；炉火完成后确认青光不属于雾谷并引向北方山口。
- 两条后续对白没有 `set_flags` 或 `emit_event_ids`，非 `DialogueRunner` 兜底路径也只在未采集时写入 `mira_met`，不会重复推进任务。
- 固定 9:30 最终图中玩家与米拉轮廓分开；采集后目标仍指向炉火，完成后目标为 `visible=false`、Alpha `0`，底部对白保持唯一叙事焦点。
- 晴天和小雨扫查均为 `21/21` 唯一哈希且 21 对同机位全部不同；三类终审均为 `PASS 0/0/0`。

## 2026-09-04 Phase 96 基线

- 恢复核对确认 `main` 与 `origin/main` 均停在 `6efe314`，当前未提交内容仅包含已通过的 Phase 95 与 Phase 96 计划记录；后台试玩 PID `63896` 仍正常响应。
- 当前主路控制点和半宽仍是 Phase 90 版本；本阶段只允许调整住宅核心 `z=-11..16` 的低频中心线与宽度节奏，必须保持南北入口、四条支路落点、道路生成/碰撞方式和 NPC 通行不变。
- 第一版参数必须依据固定 9:30 的 `08-village-north`、`10-village-south`、`11-village-west`、`12-village-far` 实图比较；不能用新增道具或植被遮挡几何问题。
- 四张基线图共同显示主路在村庄核心形成从画面近端贯穿远端的稳定浅色纵轴；`12-village-far` 最明显，三栋住宅被读成沿轴两侧排列，`11-village-west` 的支路交汇又接近规则十字。
- 调形重点应是交汇处短距离放宽、交汇之间收窄，并提高中心线偏摆的可读幅度；只做整体缩窄仍会保留“带状”轮廓，强行增加小道具则无法解决大形问题。
- 美术预审倾向更明确的 S 形和 `2.00–3.00m` 核心全宽；生活动线预审倾向保留 `2.40m+` 并把炉火交汇中心留在 `x=-0.65`。两者的分歧必须由真实接缝和远景轮廓共同裁决，不能只按数值折中。
- 当前美术候选令草甸固定生成结果从 `1902` 变为 `1898` 株，位置/明度哈希为 `530300738/961681389`，三档轮廓计数为 `633/633/632`；变化来自排除轮廓重采样，不是减少草量规则。
- 固定 9:30 晴天首轮实图中，`12-village-far` 已由等宽直带变成可辨的缓 S 形，住宅不再强烈读成沿笔直中轴两侧排布；`08/10` 的玩家脚下仍保留约三个人物横向尺度。
- `08/10/11/12` 未见四支路断缝、草甸侵入实际路面或固定车辙骑上草肩；交汇仍由既有路肩做低对比过渡。候选暂时接受，最终仍需晴雨完整扫查与三类终审。
- 首轮晴天扫查虽生成 21 张，但 `20-corner-se` 与 `21-corner-sw` 哈希完全相同且画面实际相同；两个文件时间连续，脚本日志也显示都执行过，因此是隐藏窗口下读取到上一渲染帧，不是机位内容重合。
- `grass_sweep_capture.gd` 原本只等待两个逻辑帧便读取 viewport；在读取前增加一次 `RenderingServer.frame_post_draw`，保证每个机位至少完成一帧真实绘制，再重新录制晴雨证据。
- 隐藏窗口下 `frame_post_draw` 并非可靠逐帧信号：重录在第 18 个机位等待时挂住。已废弃该方案，改用 `RenderingServer.force_draw(false)` 在读取 viewport 前同步提交当前帧，既保持隐藏运行也避免无限等待。
- 新增的支路连接断言最初错误地要求支路中心落入主路 `0.65m` 土肩，但炉火支路一直由 `VillageWearHearth` 磨损地皮跨接；当前候选边缘距离为 `1.205m` 且实图连续。契约改为锁定包含既有磨损过渡的 `1.25m` 上限，南侧车辙仍必须全部留在 `0.24m` 草甸硬排除边界内。
- 首轮终审为美术、动效 `PASS`，生活气息 `FAIL P1`：炉火支路虽有磨损地皮遮接，但晴雨近景仍能读出草缝。最终只把 `z=-5` 从 `x=-1.00/1.08m` 修至 `x=-0.65/1.25m`，同时把四支路过渡契约从临时 `1.25m` 收紧到 `0.75m`；其余四个核心点不动。
- 修复炉火接缝后的严格车辙契约发现一处南侧轮痕比 `0.24m` 草甸排除边界外露 `0.013m`；只将 `z=9` 半宽从 `1.10m` 增至 `1.12m`，让固定车辙完全留在无草带，不移动车辙或改变中心线。
- 临时探针把最大外露定位为左轮痕 `(x=-0.484, z=7.722)`，右轮痕最大仅 `0.054m`；这是弯道外侧法线偏移，最终将 `z=9` 半宽定为 `1.16m`，仍低于 Phase 90 的 `1.28m`。
- 最终参数下左/右轮痕最大边缘距离为 `0.222/0.054m`，均在 `0.24m` 无草带内；草甸为 `1912` 株，位置/明度哈希 `966575886/4154867313`，轮廓计数 `638/637/637`。

## 2026-09-04 Phase 96 完成

- 最新晴雨 `13-hearth-close` 中炉火支路与主路土肩连续重叠，原 P1 草缝已消失；轻微色差只读作踩踏磨损。
- `08/09/10/11/12` 多方向机位均确认住宅核心主路形成克制缓 S 形与宽窄变化，不再是等宽直带，也没有过度蛇行。
- 晴天和小雨扫查均为 `21/21` 唯一哈希，21 对同机位全部不同；雨线、湿润压暗、草木风摆、雾池波纹和炉火仍清楚且克制。
- 美术、生活气息和场景动效三类终审均为 `PASS 0/0/0`；最终证据归档为 `captures/phase96-*`，其中新增晴雨炉火接缝图并更新前后对照右侧为最终版本。
- 简化审查确认生产改动只涉及现有 `VillagePath` 五个核心点，不新增道具、资产、系统或依赖；捕获脚本仅补充同步绘制以保证隐藏窗口证据可靠。

## 2026-09-04 Phase 97 基线

- 当前权威提交为 `e7b3d4f`，本地 `main` 与 `origin/main` 一致且工作树起点干净；后台试玩 PID `22316` 正常响应。
- Phase 96 已关闭主路直带和炉火草缝问题，最新晴雨 21 机位及三类终审均为 `PASS 0/0/0`，这些问题不再作为开放项。
- 新一轮必须从完整序章路线的情绪与焦点接力出发重新找证据；山口和北侧雾幕按用户既定范围继续排除。
- 美术规范明确要求少量高质量焦点、植被分簇、角色轮廓清楚、生活节点有职责语义，并禁止为了丰富而新增无叙事作用的装饰。
- 主检未在四角、村庄道路或关键雨天机位发现新的角色遮挡、路肩侵入、屋顶闪烁或动效稳定性回归。
- 当前两个待交叉裁决的候选是：`02/04/07` 的石环至雾池中段道路及岔口仍偏宽、偏几何交叉面；`15-west-house` 切顶后室内仅一张桌子和大片灰地，生活信息明显弱于屋外。
- 两个候选都不能仅凭单一机位直接修改；需比较三类专项意见、运行时触发频率与现有节点复用成本，只选择更影响完整序章体验的一项。
- 动效审查返回 `none`；生活气息只提名雾池支路缺少职责落点 P2。美术审查在晴雨 `05-pond-close` 确认红叶树冠约占左侧四成、抢过冷青水面并遮住岸线，为当前唯一 P1。
- 该树已在 Phase 82 使用低饱和同源贴图，当前问题是画面面积而非色相；继续压色会削弱地域记忆点，最小根因修正应是把单树缩放从 `0.68` 收至约 `0.56`，其余树和动效链不变。
- 最终候选中 `05-pond-close` 左侧树冠仍形成酒红色异界框景，但雾池中心、玩家、岸线、三枚灯菇和住宅门窗均恢复连续可读；`04-pond-fork` 的道路与雾池关系没有出现西侧空洞。
- 固定 `time_hour=9.5` 的晴天和小雨扫查最终各为 `21/21` 唯一哈希，21 对同机位全部不同；两套文件时间均属于本轮重录。
- 将两套 Compatibility 捕获并行运行会争用同一图形设备：小雨完成 `21/21`，晴天仅写到第 6 张便退出且返回码仍为 `0`。后续视觉捕获应逐套串行运行，并同时校验 `SAVED` 行数和全部文件时间，不能只信进程退出码。

## 2026-09-04 Phase 97 完成

- 美术终审 `PASS 0/0/0`：晴雨近景中树冠视觉重量退至次级，雾池、玩家、岸线和冷青灯菇优先可读，同时保留酒红异界框景。
- 生活气息终审 `PASS 0/0/0`：住宅、雾池、主路与池边支路关系完整，西侧仍由树影、岩石、草簇和池岸填充，没有视觉空洞。
- 场景动效终审 `PASS 0/0/0`：树干、树冠、阴影及碰撞同步缩放，落叶簇中心仍与树坐标 `(-16, 17)` 一致，晴雨画面未见错位、跳闪或稳定性回归。
- 简化审查确认生产改动只有既有树表中的一个缩放值；没有新增资产、节点、系统或依赖，低优先级雾池职责道具仍保持暂缓。

## 2026-09-04 Phase 98 基线

- 最新固定 9:30 晴雨联系表及三类独立复审均没有新的 P0/P1/P2；晨间出生石环、雾池、村庄道路、住宅、药圃、炉火、角色轮廓和动态系统不应继续凭主观调整。
- `14-herb-yard` 切顶画面中的药师工作台并非未完成室内：三户已分别有 `herbalist/hearth/weaver` 身份、地毯、长凳、木箱与职业活动中心，且 Phase 62 已用真实切顶机位通过专项验收。
- 当前更有价值的未知是跨时段成片是否仍满足夜色可读、炉火/窗光/传送门成为焦点及暖村与异界未知并存；既有 `tests/time_sweep_capture.gd` 提供 8 个固定时刻/地点机位，可直接重录当前版本。

## 2026-09-04 Phase 98 完成

- 当前版本的清天天时扫查为 `8/8` 新文件、`8/8` 唯一哈希；06:00、09:30、12:00、18:12、20:30、00:00 及夜间石环/雾池均有明确固定时刻。
- 天气扫查为 `7/7` 连续新文件、`7/7` 唯一哈希；阴天、薄雾、小雨和 22:00 夜雨保持同一场景构图，天气只改变光色、雾、雨线、湿润度和风强。
- 午夜和夜雨中地面、主路、生活支路、建筑入口、玩家与 NPC 轮廓仍可辨；炉火是唯一高饱和暖焦点，窗光次级，石环冷青光和雾池灯菇在各自近景中有清楚但克制的层级。
- 三类跨时段终审均为 `PASS 0/0/0`；时间控制器使用连续插值，冒烟契约已覆盖 23:59/00:01 边界、夜间环境光下限及四类发光元素夜强昼弱，因此无需为本阶段修改生产代码。

## 2026-09-04 Phase 99 基线

- 固定 `time_hour=9.5` 的开场连续捕获已生成 `01-fade-in`、`02-arrival-ring`、`03-title-hold`、`04-world-settle`、`05-title-gone` 五帧，文件 `5/5` 唯一且脚本输出 `OPENING CAPTURE PASSED`。
- 黑场淡入、镜头拉远、地点标题和出生光环的先后关系连续；当前唯一待交叉裁决的候选是石环 `F  调查` 从世界刚淡入、出生光环仍展开时便显示，可能让普通交互提示过早介入抵达叙事。
- 此候选尚未进入生产修正；先由三类专项代理判断优先级，避免把正常的近距离可交互反馈误当成缺陷。

## 2026-09-04 Phase 99 完成

- 三类基线审查一致判定 `F  调查` 从近黑场起出现为 P1；出生光环本身短暂、面积受控且有叙事作用，不需要降亮度。
- 根因是提示刷新只依据玩家与石环距离，而出生点天然处于触发范围；最小修正以 `opening_finished` 锁住现有统一提示入口，并在地点标题完全淡出后解除门禁。
- 修复后的五帧中，前四帧无交互提示，第五帧才出现 `F  调查`；焦点顺序恢复为黑场显影、人物抵达、地点确认、世界稳定、恢复操作。
- 美术、生活气息和场景动效终审均为 `PASS 0/0/0`；没有要求修改出生环亮度、标题时长、镜头或石环特效。
- 固定 09:30 晴天与小雨扫查各为 `21/21` 唯一哈希，21 对同机位全部不同；开场门禁没有改变道路、建筑、角色、生态或天气构图。
- 简化审查确认生产改动仅为一个布尔状态、一个现有入口守卫和一个 Tween 完成回调，没有新增 UI 框架、计时器系统或交互分支。

## 2026-09-04 Phase 100 基线

- `scripts/player.gd` 的移动、镜头旋转和滚轮缩放均直接读取输入，`scripts/main.gd` 的调查输入也未检查 `opening_finished`；Phase 99 只延后了提示显示，没有真正延后玩家控制。
- 固定按住前进的运行时探针测得：起点 `(10, 14)`，1 秒后到 `(10, 7.64)`，标题完成后到 `(10, -1.14)`；开场内分别偏移 `6.36m` 与 `15.14m`，不是理论风险。
- 最小边界应只锁定玩家移动、镜头输入、缩放与调查输入；日光、天气、草木、炉火、石环及 NPC 动效必须继续播放，不能通过暂停整个场景树实现。
- 三类专项代理一致判定该问题为 P1；当前 WIP 已通过 `controls_enabled` 与 `_finish_opening()` 实现最小输入门禁，并通过完整冒烟测试。
- 用户要求在完整实机回归前暂停并提交；固定输入探针复测、开场捕获、晴雨扫查及三类终审仍是恢复开发后的首要验证项。
- 恢复后的固定输入探针输出 `OPENING_CONTROL TEST PASSED`：开场 1 秒内位置保持不变（`0.0m`），标题完全退场后同一持续前进输入产生 `2.39m` 位移；控制锁没有卡死正常游玩。
- `prologue_foundation_integration_test.gd` 模拟的是开场后的完整任务链，却未声明开场完成；新增调查门禁使其采药输入在第 55 行被正确拒绝。测试应显式进入正常游玩态，开场期间拒绝调查仍由 `smoke_test.gd` 的独立断言覆盖。

## 2026-09-04 Phase 100 完成

- 最终五帧 Compatibility 捕获固定 `time_hour=9.5`、晴天与天气种子，并在整个开场持续按住前进：地点标题与出生光环阶段位移保持 `0.0m`，标题退场后恢复移动并达到 `2.51m`。
- 三张关键帧确认焦点接力成立：角色先稳定留在出生环中心，标题与光环退场后才向石环移动，随后 `F  调查` 出现；没有通过暂停场景树牺牲环境动态。
- 晴天和小雨扫查各 `21/21` 本轮生成、`21/21` 唯一，21 对同机位全部不同；道路、建筑入口、药圃、角色、石环与村庄焦点均保持可读。
- 12 个仓库测试全部通过，另有 `TREE MOTION TEST PASSED canopies=16 leaves=12`；美术、生活气息和场景动效终审均为 `PASS 0/0/0`。

## 2026-09-04 Phase 101 基线

- 三类下一阶段审查分别提名：核心主路连续带 P2、雾池缺少职责落点 P2、雨滴缺少地面落点 P2。
- 主路已在 Phase 96 依据同类证据调整并通过 P0/P1/P2 清零；雨天地表反馈已在 Phase 70 使用湿润度与现有雾池涟漪完成，新增落点粒子会重复叠加强度。
- 雾池职责落点是 Phase 97 已明确保留、此次又被独立提名的未完成 P2。尼娅住宅工作区位于 `(-5.5, 6.5)`，雾池东南裸岸在约 `(-4.35, 10.65)`，两者可用一条不穿水、不占 PondPath 端点的短路线连接。
- 现有尼娅状态机已经覆盖工作、午间搬运、雨天避雨、炉火与夜间回家；增加一个固定下午时窗和复用同一非循环 `Interact` 动作即可闭环，无需新控制器。
- 首轮专项实图证明路线与动作成立，但 `0.18` 缩放木箱和垂直重叠湿布只读成偏白小矩形；最终证据前应放大木箱、横向错开并压暗三块布，同时后移证据机位中的玩家，避免交谈提示干扰职责阅读。

## 2026-09-04 Phase 101 完成

- 最终三帧中，木箱和灰紫、苔绿、雾青湿布保持低饱和次级层级，能读作池岸工作道具而不抢雾池冷光或角色；终点与道具保持约 `0.6-1.0m` 距离，不占道路或 PondPath。
- 尼娅按 `pond/Walk -> pond/Interact -> pond_returning/Walk` 连续活动，洗线时面向池岸目标，返程沿原路线反向；晴天时窗与雨天避雨优先级没有发现切换冲突。
- 美术、生活气息和场景动效终审全部为 `PASS 0/0/0`；Phase 101 无需继续加大道具或增加特效。

## 2026-09-04 Phase 102 基线

- 下一项必须从当前完整序章路线重新取证，不能把已通过的雾池职责、主路轮廓或雨天地表反馈再次当作开放问题。
- 复核范围继续排除山口与北侧雾幕，重点检查前中远景层次、角色/入口可读性、生活节点和异世界辨识度是否在最新 21 机位中出现新的独立缺口。
- 美术审查在 `18-corner-ne`、`19-corner-nw`、`20-corner-se`、`21-corner-sw` 提名边界巨岩浅色顶面 P2；问题集中在远端边界，且不影响主路线，本阶段暂不处理。
- 生活气息审查在小雨 `04-pond-fork`、`06-road-mid-north`、`07-road-mid-south`、`12-village-far`、`15-west-house` 发现尼娅已退到住宅墙边避雨，三块晾布却与晴天完全一致，构成稳定的职责逻辑矛盾。
- 当前天气状态本身已用连续曲线输出 `weather_rain_amount`；直接以该值交叉淡出晾布和淡入屋檐下折叠布束即可保持平滑，无需新天气状态或额外 Tween。
- 首轮实现的属性契约通过，但最小化 Compatibility 原图中最终雨态仍能看见三块 `CSGPolygon3D` 晾布；节点 `transparency=1.0` 不能单独作为像素消失证据。端点还需同步 `visible`，中间雨量继续保留两组可见和连续透明度交叉。
- 端点 `visible` 修正后的固定晴雨 `04-pond-fork` 与 `15-west-house` 已形成清楚 A/B：晴天保留灰玫、苔绿、赭黄三块低饱和晾布，小雨保留木架轮廓但移除布面；新箱筐贴近织工住宅屋檐且不侵入主路、池岸支路或尼娅站位。
- 晴雨两套 21 机位均验证最小化后台运行、退出码 `0`、`SAVED=21`、`21/21` 唯一；21 对同机位哈希全部不同。

## 2026-09-04 Phase 102 完成

- 美术、生活气息和场景动效最终复审全部为 `PASS 0/0/0`。雨量 `0 / 0.2 / 1` 的契约同时覆盖透明度和端点可见性，晴雨实图未见晾布残影或折叠布叠层。
- 本阶段只修正织工雨天收衣逻辑；边界巨岩浅色顶面仍是后续可独立处理的 P2，不纳入本次提交。

## 2026-09-04 Phase 103 基线

- 当前工作树从已推送的 `f352ec0` 干净基线开始。Phase 102 唯一明确保留项是四个角落机位中的边界巨岩浅色顶面 P2。
- 代码定位表明目标来自 `_add_boundary_scenery()` 创建的 `BoundaryScenery` 装饰岩组，与 `ValleyBoundary` 通行碰撞、山口岩组和池岸小石分离，可以做局部材质调整而不扩大行为范围。
- 四张角落原图一致显示：巨岩贴近画面边缘的大面积顶面接近日光高光色，亮度和面积共同压过角色、村庄与石环；岩体侧面和摆放本身仍能形成有效框景。
- 因此不移动或缩小岩组，也不改全局太阳和通用 `stone_material`；目标是仅给 `BoundaryScenery` 岩模型覆写低饱和蓝灰、较高粗糙度材质，保留大切面但降低远景视觉权重。
- 原 `Rocks_Diffuse.png` 同时承载青灰切面、亮灰顶面和苔色细节；实现保留这张贴图，仅以共享 `#aab4b5` 乘色压低亮面，并将粗糙度设为 `0.98`，避免把岩石改成无纹理色块。
- 首轮 `#aab4b5` 在当前 09:30 日光与环境光下仍偏轻，四角近边顶面继续接近建筑墙面高光；第二轮只把乘色调整为更明确的蓝灰 `#748084`，其余材质和场景参数不动。
- 第二轮仍未改变关键顶面，进一步追踪确认近边主体实际来自 `ValleyBoundary/CliffVisuals` 的 36 块岩体，`BoundaryScenery` 的 21 块装饰岩不是本问题主因。应撤回前者改动，把共享材质放到边界场景自己的 `_build_cliff_visuals()`。
- 正确挂到 `CliffVisuals` 后，四角原图出现明确变化：岩体成为低饱和灰蓝框景，顶面不再接近墙面或屋瓦高光，侧面仍保留层次且未落入纯黑；苔色细节和大切面继续可读。
- 小雨四角原图中，边界岩随全局天气自然降明，但轮廓和切面仍与雾、地面分离；树冠、稀疏落叶、雨线、药圃和石环焦点均保持可读。三类终审没有发现 P0/P1/P2。

## 2026-09-04 Phase 103 完成

- 最终实现只修改 `CliffVisuals` 的渲染材质，不改变 36 块岩体的位置/比例或五个不可见碰撞盒；边界继续承担舞台框景，而不成为亮色焦点。
- `#748084` 乘色、原岩石贴图和 `0.98` 粗糙度在晴雨实图中均成立；完整测试、环境动效专项和三类终审全部通过。

## 2026-09-04 Phase 104 基线

- 最新晴雨联系表未显示 Phase 103 回归。动效审查没有发现新问题；生活气息审查提名炉火屋东侧三段平行短栅栏缺少围合对象；美术审查提名雾池住宅支路中央一块旧路缘石视觉占道。
- 占道石在晴雨 `04-pond-fork` 与反向 `06-road-mid-north` 均稳定出现；`road_stones` 的固定点 `(-2.9, 0, 8)` 与后来调整过的 `VillageSouthLane` 中心线约 `(-3.2, 7.6)` 重合。
- 该石无碰撞，但会让玩家误判道路通行边界，直接违反俯视探索的道路可读性要求；本阶段只把它移到草肩外，栅栏语义问题保留为下一候选。
- 四张 `04/06` 晴雨原图确认目标是织工门前横向支路中段的灰岩，而非池岸自然石；北侧草肩在房门、晾架和主路之间留有足够空间，可将石块沿 z 轴北移约 `1.4m` 而不侵占其他活动节点。
- 现有道路距离函数计算候选点 `(-2.9, 6.0)` 距 `VillageSouthLane` 可见边缘约 `0.77m`，已越过 `0.3m` 土肩且不靠近主路；六块固定装饰石直接编号后可用契约锁定该位置，无需改动通用自然物创建接口。
- 晴天 `04-pond-fork` 显示房门支路不再被石块截断；`06-road-mid-north` 从反向确认迁移石位于草地而非道路。石块与尼娅、晾架和主路均保持可读间隔。

## 2026-09-04 Phase 104 完成

- 小雨 `04/06` 中，尼娅避雨和晾布收起逻辑保持成立，迁移石没有占用檐下站位或空晾架；支路在低对比天气下仍连续可读。
- 最终测试与三类审核均未发现 P0/P1/P2；炉火屋三段栅栏缺少围合语义仍作为下一阶段独立候选保留。
